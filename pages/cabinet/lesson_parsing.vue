...
const { data: coursesData } = await useFetch("/api/courseslesson/", {
  method: "POST",
  headers: {
...
const coursesWithLessons = computed(() => {
  if (!coursesData.value || !lessonsData.value) return [];

  const lessonsByCourse = lessonsData.value.reduce((acc, lesson) => {
    if (!acc[lesson.course]) {
      acc[lesson.course] = [];
    }
  ...
  return coursesData.value.result.map((course) => {
    return {
      ...course,
      lessons: lessonsByCourse[course.kirilica] || [],
    };
  });
});
...
const parseHtml = async () => {
  ...
  try {
    ...
    const response = await $fetch("/api/parser/parser", {
      method: "POST",
      body: { url: url.value },
    });

    lessonsArray.value = response.lessons;
  } catch (error) {
    errorMessage.value = error.data.message || "Ошибка при парсинге";
  }
};

const sendCourse = async () => {
  const lessons = lessonsArray.value.map((lesson, index) => ({
  ...
  const response = await $fetch("/api/parser/add_parser", {
    method: "POST",
    headers: {
      "Content-Type": "application/json; charset=UTF-8",
    },
    ...
    <div class="columns">
      <div class="column is-9">
        <div class="content">
          ...
          />
          <button @click="parseHtml">Парсить</button>
          ...
          <div class="field">
            ...
            >
              <el-option
                v-for="item in coursesData.result"
                :key="item._id"
                :label="item.name"
                :value="item.kirilica"
              />
          ...
          <button @click="sendCourse">Добавить</button>
          <div class="drawer-add">
            <div v-if="coursesWithLessons" class="drawer-cat-all">
              <el-collapse>
                <el-collapse-item
                  v-for="(course, index) in coursesWithLessons"
                  :key="index"
                  :title="course.title"
                >
                  <div>
                    ...
                    >
                      ...
                      <div class="drawer-cat-right">
                        ...
                        <button
                          ...
                          type="submit"
                          @click="drawerDel(lesson._id, lesson.video)"
                        >
                          Delete
                        </button>
                      </div>
